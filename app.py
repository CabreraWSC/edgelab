...
st.set_page_config(page_title="EdgeLab v7.9 隊名智能版", layout="wide")
...
st.sidebar.title("EdgeLab v7.9")
mode = st.sidebar.radio("模式", ["API 自動","CAP圖分析 (無額度用)"], key="mode79")
...
if mode == "API 自動":
    ...
    st.subheader("📊 CAP圖兩步分析 - 隊名自動對調")
    st.info("💡 步驟: 1.輸入今場主客隊名 -> 2.去馬會Cap圖Ctrl+C -> 返嚟上傳框Ctrl+V")

    c_team1, c_team2 = st.columns(2)
    with c_team1:
        home_input = st.text_input("今場主隊名 (例如 曼城 / Man City)", value=st.session_state.team_names["主隊"], key="home79")
        st.session_state.team_names["主隊"]=home_input
    with c_team2:
        away_input = st.text_input("今場客隊名 (例如 阿仙奴 / Arsenal)", value=st.session_state.team_names["客隊"], key="away79")
        st.session_state.team_names["客隊"]=away_input

    st.markdown("### 第1步：數據圖")
    ups = st.file_uploader("上傳數據圖 / 喺呢度Ctrl+V (多張得)", type=["png","jpg","jpeg"], accept_multiple_files=True, key="up79_data")
    if ups:
        for u in ups:
    ...
    with col1: st.write(f"已上傳數據圖: {len(st.session_state.data_imgs)} 張")
    with col2:
        if st.button("清空數據圖"):
            st.session_state.data_imgs=[]
            st.session_state.uploaded_ids=set()
    ...
    if st.session_state.data_imgs:
        ...
        for idx, item in enumerate(st.session_state.data_imgs):
            with cols[idx%3]:
                ...
                cat = st.selectbox(f"圖{idx+1}", ["對賽往績","主隊近期","客隊近期"], index=["對賽往績","主隊近期","客隊近期"].index(item["cat"]), key=f"cat79_{idx}")
                ...
                if st.button("X 刪", key=f"del79_{idx}"):
    ...
    with tab1:
        ...
        if s1["隊名"]["原始隊名對"]:
            swapped = sum(1 for t1,t2 in s1["隊名"]["原始隊名對"] if t1==away_input or t2==home_input and t1!=home_input)
            if swapped>0 and home_input:
                st.success(f"🔄 已自動修正 {swapped} 場主客互換，依家勝率係以 {home_input} 做主隊計")
    with tab2:
        st.write("讓球數據 - 以主客和勝率參考")
    with tab3:
        c1,c2,c3 = st.columns(3)
    ...
    with tab4:
        st.write("角球 - 請貼角球數頁，會自動計平均")

    st.divider()
    st.markdown("### 第2步：賠率圖")
    up2 = st.file_uploader("上傳賠率圖 / 喺呢度Ctrl+V", type=["png","jpg","jpeg"], accept_multiple_files=True, key="up79_odds")
    if up2:
        for u in up2:
            fid = f"{u.name}_{u.size}"
    ...
    if odds_text:
        ...
        if bodan:
            ...
            if best:
                st.metric("🏆 綜合推薦 (已修正主客)", f"{best[0]} @ {best[1]}", f"命中{best[2]:.0f}% EV+{best[3]*100:.1f}%")
                if st.button(f"入倉 ${stake_input}", key="in79"):
