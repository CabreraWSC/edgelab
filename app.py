...
st.set_page_config(page_title="EdgeLab v13.4 關鍵字修復", layout="wide")
...
def fetch_recent(tid,n):
    for url in [f"https://www.aiscore.com/api/football/team/matches?teamId={tid}&count={n}"]:
        try:
            r=requests.get(url, headers=HEADERS, timeout=10)
            if r.status_code==200:
...
def analyse(records, target):
    ...
    for r in records:
        if target.lower() not in r['主隊'].lower() and target.lower() not in r['客隊'].lower():
            # 如果對唔上，照計避免0場
            is_home=True if valid==0 else target.lower() in r['主隊'].lower()
        else:
...
st.title("🌍 v13.4 關鍵字+Drop list 修復版")
st.caption("打2字母關鍵字就搜，Drop list揀，無預設")
...
with c1:
    ...
    if st.session_state.candsA:
        ...
        sel=st.selectbox("揀A", opts, key="optA")
...
with c2:
    ...
    if st.session_state.candsB:
        ...
        sel=st.selectbox("揀B", opts, key="optB")
        st.session_state.selB=st.session_state.candsB[opts.index(sel)]

with c3:
    n=st.slider("場數",5,20,10)
    idA_man=st.number_input("A ID後備",0,999999,0)
    idB_man=st.number_input("B ID後備",0,999999,0)

teamA_obj=st.session_state.selA; teamB_obj=st.session_state.selB
teamA_name=teamA_obj['name'] if teamA_obj else ""; teamB_name=teamB_obj['name'] if teamB_obj else ""
...
if not teamA_id or not teamB_id:
    st.warning("先關鍵字搜再Drop list揀兩隊"); st.stop()
...
with st.expander("✋ 手入後備", expanded=False):
    ...
    edA=st.data_editor(dfA, num_rows="dynamic", use_container_width=True, key="edA3")
    ...
    edH=st.data_editor(dfH, num_rows="dynamic", use_container_width=True, key="edH3")
    if st.button("✅ 用手入覆蓋"):
        recA2=[]; recH2=[]
...
if st.session_state.get('statsA'):
    ...
    with c2:
        if sH: st.metric(f"② 對賽 {teamA_name} vs {teamB_name}", f"{g(sH,'勝')}勝{g(sH,'和')}和{g(sH,'負')}負", f"{g(sH,'勝率'):.0f}%")

    st.subheader("🎯 馬會主盤判斷")
    cands=[]
    cands.append({"盤口":"全場主客和","組合":f"{teamA_name} 勝","命中":g(sA,'勝率'),"原因":f"硬實力勝率 {g(sA,'勝率'):.0f}% 入{g(sA,'入'):.1f}"})
    cands.append({"盤口":"全場主客和","組合":"和","命中":g(sA,'和率'),"原因":f"和率 {g(sA,'和率'):.0f}% 共{g(sA,'和')}場"})
    cands.append({"盤口":"全場主客和","組合":f"{teamB_name} 勝","命中":g(sA,'負率'),"原因":f"{teamA_name}負率 {g(sA,'負率'):.0f}%"})
    if sH: cands.append({"盤口":"全場主客和(對賽)","組合":f"{teamA_name} 勝(對賽)","命中":g(sH,'勝率'),"原因":f"對賽 {g(sH,'勝')}勝 勝率{g(sH,'勝率'):.0f}%"})

    cands.append({"盤口":"半場主客和","組合":f"半場 {teamA_name} 勝","命中":g(sA,'半勝率') if g(sA,'半勝率')>0 else g(sA,'勝率')*0.55,"原因":f"半勝率 {g(sA,'半勝率'):.0f}%"})
    cands.append({"盤口":"半場主客和","組合":"半場 和","命中":g(sA,'半和率') if g(sA,'半和率')>0 else 35,"原因":f"半和 {g(sA,'半和率'):.0f}%"})
    cands.append({"盤口":"半場主客和","組合":f"半場 {teamB_name} 勝","命中":g(sA,'半負率') if g(sA,'半負率')>0 else g(sA,'負率')*0.55,"原因":f"半負 {g(sA,'半負率'):.0f}%"})

    cands.append({"盤口":"入球大細","組合":"大1.5","命中":g(sA,'大1.5%'),"原因":f"大1.5 {g(sA,'大1.5%'):.0f}% 總入{g(sA,'總入'):.1f}"})
    cands.append({"盤口":"入球大細","組合":"大2.5","命中":g(sA,'大2.5%'),"原因":f"大2.5 {g(sA,'大2.5%'):.0f}%"})
    cands.append({"盤口":"入球大細","組合":"細2.5","命中":100-g(sA,'大2.5%'),"原因":f"細2.5 {100-g(sA,'大2.5%'):.0f}%"})

    if sA.get('波膽'):
        for sc,cnt in sA.get('波膽',[])[:3]:
            cands.append({"盤口":"全場波膽","組合":f"波膽 {sc}","命中":cnt/g(sA,'場')*100 if g(sA,'場')>0 else 0,"原因":f"出現{cnt}次"})
    ...
    for mk in ["全場主客和","半場主客和","入球大細","全場波膽"]:
        ...
        for i,c in enumerate(subset):
            with cols[i]:
                odd=st.number_input(f"{c['組合']}", 1.05, 50.0, 2.0, 0.05, key=f"odd_{mk}_{i}_{teamA_name}_fix")
