...
st.set_page_config(page_title="EdgeLab v16.3 合理推薦", layout="wide")
...
def analyse(records,target):
    ...
    return {"f":valid,"w_rate":w/valid*100,"o25":o25/valid*100,"scores":scores,"w":w}
...
def reasonable_score(hit, ev_val):
    ...
    if hit < 15 and ev_val < 50: return -999 # 波膽太冷門唔推
...
with tab_pre:
    ...
    if sH and sA:
        ...
        bets=[(f"主勝 @ {oh}",pre_home,ev(pre_home,oh),f"往績{home}勝率{sH['w_rate']:.0f}% > {away}，對賽有優勢"),(f"大2.5 @ {oo}",pre_o25,ev(pre_o25,oo),f"兩隊近10場平均總入球{(sH['f']+sA['f'])/20:.1f}，大2.5 {pre_o25:.0f}%")]
        best=make_recommend(bets)
        if best: st.success(f"🏆 賽前合理推薦：{best[0]} 命中{best[1]:.0f}% EV {best[2]:.1f}%\n原因：{best[3]}")
        else: st.warning("賽前無合理值博，命中率<35%或EV為負")
...
with tab_live:
    ...
    def valid_scores(ch,ca): return [f"{i}-{j}" for i in range(ch,ch+4) for j in range(ca,ca+4) if i<=5 and j<=5]

    v1,v2=st.tabs(["版本A 純即場","版本B 綜合"])
    with v1:
        prob_o25=min(88,25+(h_on+a_on)*8+(cur_h+cur_a)*12); prob_h=max(5,min(95,h_dom+(cur_h-cur_a)*10))
        o_o25=st.number_input("大2.5賠率 A",0.0,100.0,1.85,0.05,key="v1_o25"); o_h=st.number_input("主勝賠率 A",0.0,100.0,2.6,0.05,key="v1_h"); o_a=st.number_input("客勝賠率 A",0.0,100.0,2.8,0.05,key="v1_a")
        vlist=valid_scores(cur_h,cur_a)
        bets=[]
        bets.append((f"大2.5 @ {o_o25}",prob_o25,ev(prob_o25,o_o25),f"即場射正{h_on+a_on}次，Dangerous {h_datt+a_datt}次，入球預期高，已入{cur_h+cur_a}球"))
        bets.append((f"主勝 @ {o_h}",prob_h,ev(prob_h,o_h),f"主隊攻勢{h_dom:.0f}%，Attacks {h_att} vs {a_att}，Dangerous {h_datt} vs {a_datt}壓制"))
        bets.append((f"客勝 @ {o_a}",100-prob_h,ev(100-prob_h,o_a),f"客隊射正{a_on}次反擊效率高，客隊表現較好"))
        cols=st.columns(4)
        for idx,cs in enumerate(vlist[:8]):
            oo=st.number_input(f"{cs}賠率 A",0.0,200.0,8.0,0.5,key=f"v1_{cs}")
            p=max(2,20-abs((int(cs.split('-')[0]+int(cs.split('-')[1]))-(cur_h+cur_a+1))*5)
            evv=ev(p,oo)
            # 波膽如果同攻勢相反，降命中
            if int(cs.split('-')[0])>int(cs.split('-')[1]) and h_dom<45: p*=0.5
            if int(cs.split('-')[0])<int(cs.split('-')[1]) and h_dom>55: p*=0.5
            bets.append((f"波膽 {cs} @ {oo}",p,evv,f"而家{cur_h}-{cur_a} {minute_live}'，{cs}最貼近即場走勢"))
            with cols[idx%4]: st.caption(f"{cs} {p:.0f}% EV {evv:.0f}%")

        best=make_recommend(bets)
        if best: st.success(f"🏆 A合理推薦：{best[0]}\n命中 {best[1]:.0f}% | EV {best[2]:.1f}% | 合理分 {reasonable_score(best[1],best[2]):.0f}\n原因：{best[3]}")
        else: st.warning("A版暫無合理投注，全部命中<35%或EV<0，建議等下一分鐘")

    with v2:
        sH=st.session_state.get('preH'); sA=st.session_state.get('preA')
        if not sH: st.warning("先捉往績")
        else:
            pre_home=sH['w_rate']*0.6+(100-sA['w_rate'])*0.4; pre_o25=(sH['o25']+sA['o25'])/2
            prob_home_v2=max(5,min(95,pre_home*0.4+h_dom*0.6+(cur_h-cur_a)*8)); prob_o25_v2=min(88,pre_o25*0.4+(25+(h_on+a_on)*8+(cur_h+cur_a)*12)*0.6)
            o=st.number_input("大2.5賠率 B",0.0,100.0,1.85,0.05,key="v2_o25"); o2=st.number_input("主勝賠率 B",0.0,100.0,2.6,0.05,key="v2_h")
            bets2=[(f"大2.5 @ {o}",prob_o25_v2,ev(prob_o25_v2,o),f"往績大2.5 {pre_o25:.0f}% + 即場Dangerous {h_datt+a_datt}次，綜合預期"),
                   (f"主勝 @ {o2}",prob_home_v2,ev(prob_home_v2,o2),f"往績主勝{pre_home:.0f}% + 即場攻勢{h_dom:.0f}%壓制，表現一致")]
            best2=make_recommend(bets2)
            if best2: st.success(f"🏆 B綜合合理推薦：{best2[0]}\n命中 {best2[1]:.0f}% | EV {best2[2]:.1f}%\n原因：{best2[3]}")
