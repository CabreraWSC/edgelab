...
st.set_page_config(page_title="EdgeLab v13.7 先入賠率校正", layout="wide")
...
def search_teams(kw):
    ...
    for url in [f"https://www.aiscore.com/api/search?query={kw}"]:
        try:
...
def parse(m):
    try:
        ...
        if isinstance(m.get('halfScore'),dict): hhs=m.get('halfScore',{}).get('home',0); haws=m.get('halfScore',{}).get('away',0)
        if hs==0 and aw==0:
...
st.title("🌍 v13.7 先入馬會賠率再校正分析")
st.caption("流程: 1.關鍵字搜隊 2.先入馬會主客和賠率 3.自動捉 4.賠率校正後分析更準")
...
st.divider()
st.subheader("① 先入馬會全場主客和賠率 (用嚟校正分析)")
st.caption("點一下格直接打新數，唔使Delete，打完先捉，分析會更準")
...
try:
    ...
    st.info(f"馬會隱含機率(去水): {teamA_name}勝 {imp1:.1f}% | 和 {impX:.1f}% | {teamB_name}勝 {imp2:.1f}% | 水位 {over*100-100:.1f}%")
except:
    imp1=impX=imp2=33.3

st.divider()
col1,col2,c3=st.columns([1,1,1])
with col1: n=st.slider("捉幾多場",5,20,10)
with col2:
...
if not st.session_state.get('statsA'):
    st.warning("入完賠率後按捉波，會用賠率校正分析"); st.stop()
...
# 校正: 數據60% + 馬會隱含30% + 對賽10% -> 更貼市
win_data=weighted(g(sA,'勝率'), g(sH,'勝率'))
draw_data=weighted(g(sA,'和率'), g(sH,'和率'))
lose_data=weighted(g(sA,'負率'), g(sH,'負率'))

# 最終命中 = 數據50% + 市場50%
final_win = win_data*0.5 + imp1*0.5
final_draw = draw_data*0.5 + impX*0.5
final_lose = lose_data*0.5 + imp2*0.5
...
st.subheader("🎯 校正後分析 (數據+對賽+馬會賠率)")
cands=[]
cands.append({"盤口":"全場主客和","組合":f"{teamA_name} 勝","數據命中":f"{win_data:.0f}%","市場隱含":f"{imp1:.0f}%","校正後命中":f"{final_win:.1f}%","命中值":final_win,"原因":f"硬{g(sA,'勝率'):.0f}%+對賽{g(sH,'勝率') if sH else 0:.0f}% vs 馬會{imp1:.0f}% -> {final_win:.0f}%"})
cands.append({"盤口":"全場主客和","組合":"和","數據命中":f"{draw_data:.0f}%","市場隱含":f"{impX:.0f}%","校正後命中":f"{final_draw:.1f}%","命中值":final_draw,"原因":f"硬{g(sA,'和率'):.0f}%+對賽{g(sH,'和率') if sH else 0:.0f}% vs 馬會{impX:.0f}% -> {final_draw:.0f}%"})
cands.append({"盤口":"全場主客和","組合":f"{teamB_name} 勝","數據命中":f"{lose_data:.0f}%","市場隱含":f"{imp2:.0f}%","校正後命中":f"{final_lose:.1f}%","命中值":final_lose,"原因":f"硬{g(sA,'負率'):.0f}%+對賽{g(sH,'負率') if sH else 0:.0f}% vs 馬會{imp2:.0f}% -> {final_lose:.0f}%"})

# 其他盤口
ht_win=weighted(g(sA,'半勝率'), g(sH,'半勝率')); ht_draw=weighted(g(sA,'半和率'), g(sH,'半和率')); ht_lose=weighted(g(sA,'半負率'), g(sH,'半負率'))
cands.append({"盤口":"半場主客和","組合":f"半場 {teamA_name} 勝","數據命中":f"{ht_win:.0f}%","市場隱含":"-","校正後命中":f"{ht_win:.0f}%","命中值":ht_win,"原因":f"半場勝率"})
cands.append({"盤口":"半場主客和","組合":"半場 和","數據命中":f"{ht_draw:.0f}%","市場隱含":"-","校正後命中":f"{ht_draw:.0f}%","命中值":ht_draw,"原因":f"半場和"})
cands.append({"盤口":"入球大細","組合":"大2.5","數據命中":f"{g(sA,'大2.5%'):.0f}%","市場隱含":"-","校正後命中":f"{g(sA,'大2.5%'):.0f}%","命中值":g(sA,'大2.5%'),"原因":f"場均{g(sA,'總入'):.1f}球"})
cands.append({"盤口":"入球大細","組合":"細2.5","數據命中":f"{100-g(sA,'大2.5%'):.0f}%","市場隱含":"-","校正後命中":f"{100-g(sA,'大2.5%'):.0f}%","命中值":100-g(sA,'大2.5%'),"原因":f"細2.5"})

df_show=pd.DataFrame(cands)
st.dataframe(df_show, use_container_width=True, hide_index=True)

st.divider()
st.subheader("💰 EV (用你入嘅賠率計校正後)")
ev_rows=[]
for c in cands:
    if c['盤口']!="全場主客和": continue
    # 搵返對應賠率
    odd=1.95
    if "和"==c['組合']: odd=oX
    elif teamA_name in c['組合']: odd=o1
    else: odd=o2
    ev=(c['命中值']/100*odd-1)*100
    grade="🔥 超值" if ev>15 else "✅ 值博" if ev>5 else "⚠️ 一般" if ev>-5 else "❌ 唔值"
    ev_rows.append({"組合":c['組合'],"校正命中":f"{c['命中值']:.1f}%","你入賠率":odd,"EV":f"{ev:.1f}%","評級":grade,"解讀":c['原因']})

st.dataframe(pd.DataFrame(ev_rows), use_container_width=True, hide_index=True)
if ev_rows:
    best=max(ev_rows, key=lambda x: float(x['EV'].replace('%','')))
    st.success(f"🏆 你入完賠率後最值博: {best['組合']} 命中{best['校正命中']} @ {best['你入賠率']} EV {best['EV']} {best['評級']}")
