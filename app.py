...
st.set_page_config(page_title="EdgeLab v5.2", layout="wide")
...
st.title(f"EdgeLab v5.2 - 投注額+回報")
...
for g in games:
    ...
    best_of_groups=[max(markets, key=lambda x: x["prob"]) for markets in groups.values()]
    ...
    with st.container(border=True):
        ...
        for idx,(gname,markets) in enumerate(groups.items()):
            with cols[idx]:
                ...
                tmp=[]
                for m in markets:
                    tmp.append([m["pick"], m["odds"], f"{m['prob']*100:.1f}%", f"+{m['ev']*100:.1f}%", m["ret"]])
                st.table(pd.DataFrame(tmp, columns=["投注","賠率","命中","EV","回報$"]))

        st.success(f"推薦: {final['pick']} @ {final['odds']} | 命中 {final['prob']*100:.1f}% | ${stake_input} -> ${final['ret']}")
...
# 紀錄區 - 用st.table避開pyarrow bug
st.divider()
st.subheader("投注紀錄")
if st.session_state.bets:
    df=pd.DataFrame(st.session_state.bets)
    # 用table唔用dataframe, 避開Duplicate column bug
    st.table(df)
    
    total_stake=sum([b["投注額"] for b in st.session_state.bets])
    total_return=sum([b["回報額"] for b in st.session_state.bets])
    total_profit=sum([b["預期盈利"] for b in st.session_state.bets])
    
    m1,m2,m3=st.columns(3)
    m1.metric("總投注額", f"${total_stake}")
    m2.metric("總回報額", f"${total_return}")
    m3.metric("預期盈利", f"${total_profit:.0f} (ROI {total_profit/total_stake*100:.1f}%)")
