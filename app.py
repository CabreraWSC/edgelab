...
st.set_page_config(page_title="EdgeLab v13.5 合併最終", layout="wide")
...
def analyse(records, target):
    ...
    win=draw=lose=gf=ga=btts=o25=o15=o35=0; ht_w=ht_d=ht_l=0; scores=Counter()
    valid=0
    for r in records:
        is_home=target.lower() in r['主隊'].lower() if target.lower() in r['主隊'].lower() or target.lower() in r['客隊'].lower() else True
...
st.title("🌍 v13.5 合併+直接打數")
st.caption("關鍵字搜:Drop list揀 | 自動捉為主 | 盤口已合併無重複 | 表格直接打賠率唔使Delete")

c1,c2,c3=st.columns([2,2,1])
with c1:
    kwA=st.text_input("主隊A關鍵字", placeholder="Swin / Arse", key="kwA")
...
if not teamA_obj or not teamB_obj:
    st.warning("先搜再揀兩隊"); st.stop()

teamA_name=teamA_obj['name']; teamB_name=teamB_obj['name']; teamA_id=teamA_obj['id']; teamB_id=teamB_obj['id']
...
with st.expander("✋ 手入後備 (捉唔到先用)", expanded=False):
    ...
    edA=st.data_editor(dfA, num_rows="dynamic", use_container_width=True, key="edA4")
    ...
    edH=st.data_editor(dfH, num_rows="dynamic", use_container_width=True, key="edH4")
    if st.button("✅ 用手入覆蓋"):
        recA2=[]; recH2=[]
...
if st.session_state.get('statsA'):
    ...
    cands=[]
    cands.append({"盤口":"全場主客和","組合":f"{teamA_name} 勝","命中":win_rate,"原因":f"硬{g(sA,'勝率'):.0f}%+對賽{g(sH,'勝率') if sH else 0:.0f}%加權->{win_rate:.0f}% 入{g(sA,'入'):.1f}"})
    ...
    cands.append({"盤口":"半場主客和","組合":f"半場 {teamA_name} 勝","命中":ht_win,"原因":f"半場勝 硬{g(sA,'半勝率'):.0f}%+對賽{g(sH,'半勝率') if sH else 0:.0f}%->{ht_win:.0f}%"})
    ...
    cands.append({"盤口":"入球大細","組合":"大2.5","命中":weighted(g(sA,'大2.5%'), g(sH,'大2.5%') if sH else 0),"原因":f"大2.5 硬{g(sA,'大2.5%'):.0f}%+對賽{g(sH,'大2.5%') if sH else 0:.0f}%"})
    cands.append({"盤口":"入球大細","組合":"細2.5","命中":100-weighted(g(sA,'大2.5%'), g(sH,'大2.5%') if sH else 0),"原因":f"細2.5"})
    if sA.get('波膽'):
        for sc,cnt in sA.get('波膽',[])[:3]:
            cands.append({"盤口":"全場波膽","組合":f"波膽 {sc}","命中":cnt/g(sA,'場')*100,"原因":f"出現{cnt}次"})
    if sH and sH.get('波膽'):
        for sc,cnt in sH.get('波膽',[])[:2]:
            cands.append({"盤口":"全場波膽","組合":f"對賽波膽 {sc}","命中":cnt/g(sH,'場')*100,"原因":f"對賽出現{cnt}次"})

    st.subheader("🎯 馬會主盤 (已合併，無重複)")
    st.dataframe(pd.DataFrame(cands), use_container_width=True, hide_index=True)

    st.divider()
    st.subheader("💰 貼馬會賠率 - 點一下直接打新數，唔使Delete")
    st.caption("表格可直接編輯賠率，打完自動計EV")

    # 準備EV表格，可直接打賠率
    ev_df=pd.DataFrame([
        {"盤口":c["盤口"],"組合":c["組合"],"命中率%":round(c["命中"],1),"馬會賠率":2.00,"原因":c["原因"]}
    ...
    edited_ev=st.data_editor(
        ...
        column_config={
            ...
            "命中率%": st.column_config.NumberColumn(disabled=True),
            "馬會賠率": st.column_config.NumberColumn(min_value=1.01, max_value=100.0, step=0.05, format="%.2f"),
    ...
    for _,r in edited_ev.iterrows():
        ev=(r["命中率%"]/100*r["馬會賠率"]-1)*100
        grade="🔥 超值" if ev>15 else "✅ 值博" if ev>5 else "⚠️ 一般" if ev>-5 else "❌ 唔值"
        result.append({"盤口":r["盤口"],"組合":r["組合"],"命中":f"{r['命中率%']:.0f}%","賠率":r["馬會賠率"],"EV":f"{ev:.1f}%","評級":grade,"原因":r["原因"]})

    st.dataframe(pd.DataFrame(result), use_container_width=True, hide_index=True)
    # 高亮最值博
    if result:
        best=max(result, key=lambda x: float(x['EV'].replace('%','')))
        if float(best['EV'].replace('%',''))>5:
            st.success(f"🏆 最值博: {best['組合']} @ {best['賠率']} EV {best['EV']} {best['評級']} | {best['原因']}")
